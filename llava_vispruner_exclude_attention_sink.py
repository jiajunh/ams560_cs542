...
def apply_vispruner(inputs, model, args):
    ...
    sorted_idx = torch.argsort(cls_attention, descending=True)
    sorted_vals = cls_attention[sorted_idx]
    sink_mask = sorted_vals < args.attention_sink_threshold
    non_sink_idx = sink_mask.nonzero(as_tuple=False)[0].item()
    
    important_token_idx = sorted_idx[non_sink_idx:non_sink_idx+args.important_tokens]
...
def eval_pope(model, tokenizer, processor, args):
    ...
    with open(args.output_file, "a", encoding="utf-8") as f:
        ...
        f.write(f"Attention sink threshold: {args.attention_sink_threshold}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--attention_sink_threshold', type=float, default=0.02)
